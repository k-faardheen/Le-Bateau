...
const getAnswer = async () => {
  const response = await fetch("./json/css-exercises.json");
  ...
  btn.addEventListener("click", (e) => {
    ...
    if (e.target.className === "try-again") {
      ...
      for (i = 0; i < inputBox.length; i++) {
        flag = true;
        if (inputBox[i].value.toLowerCase() !== val[parent.id][i]) {
          //Check input value of user against answer in the JSON file.
